...
class List:
    ...
    def append(self, value):
        ...
        if self._front is None:
            ...
            # this is saying that self._rear is linked to the following new_node
            self._rear._next = new_node
            self._rear = new_node

        self._count += 1

        # so the ._next allows me to link the nodes
        return
    ...
    def find(self, key):
        ...
        -------------------------------------------------------
        """
        k, current, j = self._linear_search(key)
    ...
    def index(self, key):
        ...
        -------------------------------------------------------
        """
        i, j, index = self._linear_search(key)
    ...
    def max(self):
        ...
        current = self._front
        maxi = self._front._value

        while current is not None:
            if current._value > maxi:
                maxi = current._value
                current = current._next
            else:
                current = current._next
    ...
    def min(self):
        ...
        current = self._front
        mini = self._front._value

        while current is not None:
            if current._value < mini:
                mini = current._value
                current = current._next
            else:
                current = current._next
    ...
    def reverse(self):
        ...
        # reverses links between the nodes and then moves up
    ...
    def _intersection_r_aux(self, check, source2):
        if check is not None:
            check2 = check._value
            prev, curr, index = source2._linear_search(check2)
            if curr is not None:
                prev, curr, index = self._linear_search(check2)
                if curr is None:
                    self.append(check2)
            self._intersection_r_aux(check._next, source2)
        return
